...
router.put("/approve/:courseId", async function (req, res, next) {
  ...
  if (
    ...
    ["EMPLOYEE", "ADMIN"].includes(req.user.roles.name)
  ) {
    return res.status(401).json({ error: "not authorized to approve courses" });
  } else if (
    currentStatus == "AWAITING_APPROVAL" &&
    ["EMPLOYEE", "SUPER_ADMIN"].includes(req.user.roles.name)
  ) {
    return res
      .status(401)
      .json({ error: "not authorized to send for approval" });
  }
...
router.delete('/delete/:courseId', async function(req, res, next) {
  var courseID = +req.params.courseId;
  try {
    const deleteTopics = await prisma.topic.deleteMany({
      where: {
        courseID: courseID
      }
    }).catch();
